...
function Linux.RegisterAccentElement(element, isActive)
    if isActive == nil then isActive = true end
    
    for i, data in pairs(Linux.AccentElements) do
    ...
    table.insert(Linux.AccentElements, {element = element, isActive = isActive})
end

function Linux.UnregisterAccentElement(element)
    for i, data in pairs(Linux.AccentElements) do
        if data.element == element then
...
function Linux.UpdateAccentElementState(element, isActive)
    for i, data in pairs(Linux.AccentElements) do
        if data.element == element then
            data.isActive = isActive
            break
...
function Linux.ApplyTheme(themeName)
    local success, errorMsg = pcall(function()
        if not Linux.Themes[themeName] then 
            warn("Linux UI: Theme '" .. tostring(themeName) .. "' not found")
            return 
        end
        ...
        for _, data in pairs(Linux.AccentElements) do
            if data.element and data.element.Parent and data.isActive then
                local tweenInfo = TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
                local tween = TweenService:Create(data.element, tweenInfo, {BackgroundColor3 = newColor})
                tween:Play()
            end
        end
    end)
    ...
    if not success then
        warn("Linux UI: Error applying theme - " .. tostring(errorMsg))
    end
end
...
function Linux.SaveThemeSettings()
    ...
    if not success then
        warn("Linux UI: Failed to save theme settings - " .. tostring(result))
    end
...
function Linux.LoadThemeSettings()
    ...
    if not success then
        warn("Linux UI: Failed to load theme settings - " .. tostring(result))
    end
...
function Linux:SafeCallback(Function, ...)
    ...
    if not Success then
        ...
        warn("Linux UI: Callback Error - " .. errorText)
        self:Notify({
...
function Linux:Notify(config)
    local success, errorMsg = pcall(function()
        ...
        Linux.RegisterAccentElement(ProgressFill, true)
    ...
    if not success then
        warn("Linux UI: Failed to create notification - " .. tostring(errorMsg))
    end
end
...
function Linux.SaveAutoLoadSettings()
    ...
    if not success then
        warn("Linux UI: Failed to save auto load settings - " .. tostring(result))
    end
...
function Linux.LoadAutoLoadSettings()
    ...
    if not success then
        warn("Linux UI: Failed to load auto load settings - " .. tostring(result))
    end
...
function Linux.SavePrivacySettings()
    ...
    if not success then
        warn("Linux UI: Failed to save privacy settings - " .. tostring(result))
    end
...
function Linux.LoadPrivacySettings()
    ...
    if not success then
        warn("Linux UI: Failed to load privacy settings - " .. tostring(result))
    end
...
function Linux.SaveConfig(name)
    if not name or name == "" then
        warn("Linux UI: Invalid configuration name")
    ...
    if success and result == true then
        ...
        warn("Linux UI: Failed to save config - " .. tostring(result))
...
function Linux.LoadConfig(name)
    ...
    if not success then
        warn("Linux UI: Failed to load config - " .. tostring(result))
    end
...
function Linux.DeleteConfig(name)
    ...
    if success and result == true then
        ...
        warn("Linux UI: Failed to delete config - " .. tostring(result))
...
function Linux.LoadConfigList()
    ...
    if not success then
        warn("Linux UI: Failed to load config list - " .. tostring(result))
    end
...
function Linux.Create(config)
    ...
    if not success then
        warn("Linux UI: Failed to create UI - " .. tostring(errorMsg))
        return nil
    end
...
function LinuxLib.Tab(config)
    ...
    Linux.RegisterAccentElement(TabAccentBar, false)
    ...
    local function SelectTab()
        ...
        for currentTabIndex, currentTab in pairs(Tabs) do
            if currentTabIndex ~= tabIndex then
                currentTab.Content.Visible = false
                currentTab.TitleFrame.Visible = false
                currentTab.ContainersWrapper.Visible = false
                ...
                currentTab.Button.BorderSizePixel = 0
        
                if currentTab.Icon then
                    currentTab.Icon.ImageColor3 = Color3.fromRGB(150, 150, 150)
                end
                TweenService:Create(currentTab.TabAccentBar, tweenInfoShrink, {Size = UDim2.new(0, 0, 0, 24)}):Play()
                Linux.UpdateAccentElementState(currentTab.TabAccentBar, false)
            end
        end
        ...
        Linux.UpdateAccentElementState(TabAccentBar, true)
        CurrentTab = tabIndex
        ...
        local function UpdateToggle(thisToggle)
            ...
            if thisTrack and thisKnob then
                ...
                if currentState then
                    ...
                    Linux.RegisterAccentElement(thisTrack, true)
                else
                    ...
                    Linux.RegisterAccentElement(thisTrack, false)
                end
            end
        ...
        local function PopulateOptions()
            ...
            if IsOpen then
                for i, opt in pairs(Options) do
                    ...
                    Linux.RegisterAccentElement(OptAccentBar, isSelected)
                                    ...
                                    if btn:IsA("TextButton") then
                                        ...
                                        if btn.Text == tostring(opt) then
                                            ...
                                            if btnAccentBar then
                                                ...
                                                Linux.UpdateAccentElementState(btnAccentBar, true)
                                            end
                                        else
                                            btn.TextColor3 = Color3.fromRGB(200, 200, 210)
                                            if btnAccentBar then
                                                TweenService:Create(btnAccentBar, tweenInfo, {Size = UDim2.new(0, 0, 0, 20)}):Play()
                                                Linux.UpdateAccentElementState(btnAccentBar, false)
                                            end
                                        end
                                    end
                                end
    ...
    function TabElements.Slider(config)
        ...
        Linux.RegisterAccentElement(FillBar, true)
                ...
                UpdateConnection = RunService.Heartbeat:Connect(function()
                    ...
                    if currentTime - lastUpdate >= updateInterval then
                        ...
                        elseif not success then
                            warn("Linux UI: Label update callback error - " .. tostring(newText))
                        end
